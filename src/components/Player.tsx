...
export const Player = () => {
  ...
  const {isPlay, songPath, songTime, volume, songId} = usePlayStore(state => state.currentPlay);
  ...
  useEffect(() => {
    fetch(
      `http://localhost:4321/api/song/${songId}`, {
      method: "GET",
      headers: {
    ...
    <div className={cn("w-full h-full pt-2 items-center", styles.contenido)}>
      {
        ...
        ? (
          <div className="flex justify-center items-center gap-3 justify-self-start">
            <picture>
              <img src={song.album.cover} alt="album image" className="w-14 h-14 rounded-lg shadow-xl shadow-neutral-900"/>
            </picture>
            <div>
              <p className="text-sm font-circular-medium">{song.name}</p>
              <p className="text-xs font-circular-light text-neutral-400">{song.artist.name}</p>
            </div>
          </div>
        )
        : <div></div>
      ...
      <div className="flex flex-col gap-2 justify-center items-center">
        ...
        <div className="flex justify-center items-center gap-x-2">
          ...
          <Slider onValueChange={(time) => handlerSongTime(time)} value={[songTime]} defaultValue={[audioRef?.currentTime || 0]} max={audioRef?.duration} step={1} className="w-[200px] sm:w-[250px] lg:w-[400px]"/>
      ...
      <div className="flex items-center gap-2 justify-self-end">
        <svg 
          ...
          aria-hidden="true" id="volume-icon" 
          viewBox="0 0 16 16" 
        ...
        <Slider onValueChange={(volume) => handlerSongVolume(volume)} defaultValue={[volume*100]} max={100} step={1} className="w-[80px] sm:w-[100px] lg:w-[120px]"/>
      </div>
      <audio ref={refAudio}/>
    </div>
  )
